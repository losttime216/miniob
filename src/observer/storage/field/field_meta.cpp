...
RC FieldMeta::init(const char *name, AttrType attr_type, int attr_offset, int attr_len, bool visible, int field_id, bool nullable)
{
  if (common::is_blank(name)) {
   ...
   if (AttrType::UNDEFINED == attr_type || attr_offset < 0 || attr_len <= 1) { // attr_len 包含了标记位的长度 所以不能小于等于1
...
bool FieldMeta::is_nullable() const { return nullable_; }
...
bool FieldMeta::check_null_marker(const char *record_data) const
{
  return record_data[attr_offset_ + attr_len_ - 1] == 1;
}

RC FieldMeta::set_null_marker(char *record_data, bool is_null) const {
  if (is_null) {
    record_data[attr_offset_ + attr_len_ - 1] = 1; // 因为从 0 开始，且 attr_len 包含了标记位的长度，所以是 attr_len_ - 1
  } else {
    record_data[attr_offset_ + attr_len_ - 1] = 0;
  }
  return RC::SUCCESS;
}
